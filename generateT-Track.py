...
"""
Description:
  Auto Generate G-Code for milling T-Track 
  in the spoil board(grbl 1.1 control)
...
def operation(length,feed,zFeed, thickness):
  ...
  while zStep <= partThickness:
    zStep2=zStep+passAmount
    zPos2 = '%.3f' % round(zStep2, 3) # Round to 3 decimal places
    output = (
      f"\n(PASSES {passes})\n"
      f"G01 G90 X0 Y0 F{feed}\n"
      f"G01 Z-{zPos} F{zFeed}\n"
      f"G01 Y{y_move} F{feed}\n"
      f"G01 Z-{zPos2} F{zFeed}\n"
      f"G01 Y0 F{feed}\n"
    )
    zStep += passAmount*2
